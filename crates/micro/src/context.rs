	...
	loop {
		...
		let main_canvas_transform = main_canvas.as_ref().map(|canvas| {
			main_canvas_transform(
				canvas.size(),
				window_size(),
				Context::with(|ctx| ctx.integer_scaling_enabled),
			)
		});
		...
		let egui_output = egui_ctx.run_ui(egui_input, |ui| {
			if let DevToolsState::Enabled { visible } = Context::with(|ctx| ctx.dev_tools_state) {
				Panel::top("main_menu").show(ui, |ui| {
					egui::MenuBar::new().ui(ui, |ui| {
						app.debug_menu(ui);
						ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
							if let Some(stats) = app.debug_stats() {
			...
			.filter_map(Event::from_sdl3_event)
		{
			...
			app.event(event.transform_mouse_events(mouse_event_transform));
		}
		drop(span);
...
pub fn monitor_resolution() -> Result<UVec2, sdl3::Error> {
	let display_mode = Context::with(|ctx| -> Result<sdl3::video::DisplayMode, sdl3::Error> {
		ctx.window.get_display()?.get_mode()
	})?;
...
pub fn integer_scaling_enabled() -> bool {
	Context::with(|ctx| ctx.integer_scaling_enabled)
}

/// Sets the window mode (windowed or fullscreen).
pub fn set_window_mode(window_mode: WindowMode) -> Result<(), sdl3::Error> {
	match window_mode {
		WindowMode::Fullscreen => Context::with_mut(|ctx| ctx.window.set_fullscreen(true)),
		WindowMode::Windowed { size } => Context::with_mut(|ctx| -> Result<(), sdl3::Error> {
...
/// context was set up to use a main canvas.
pub fn set_integer_scaling_enabled(enabled: bool) {
	Context::with_mut(|ctx| ctx.integer_scaling_enabled = enabled)
}
...
/// ahead of time.
pub fn desired_maximum_frame_latency() -> u32 {
	Context::with(|ctx| ctx.graphics.desired_maximum_frame_latency())
}
...
/// input lag.
pub fn set_desired_maximum_frame_latency(frames: u32) {
	Context::with_mut(|ctx| ctx.graphics.set_desired_maximum_frame_latency(frames));
}
...
pub fn set_clear_color(color: impl Into<LinSrgb>) {
	Context::with_mut(|ctx| {
		...
		if ctx.main_canvas_size.is_none() {
			ctx.graphics.clear_color = color;
		}
	});
}
...
pub fn mouse_position() -> Vec2 {
	let mouse_state = Context::with(|ctx| ctx.event_pump.mouse_state());
	let transform = Context::with(|ctx| ctx.main_canvas_size)
		.map(|size| {
			main_canvas_transform(
				size,
				window_size(),
...
pub fn mouse_wheel_delta() -> Vec2 {
	Context::with(|ctx| ctx.mouse_wheel_delta)
}

/// Gets the IDs of the currently connected gamepads.
pub fn connected_gamepad_ids() -> Result<Vec<GamepadId>, sdl3::Error> {
	Context::with(|ctx| {
		Ok(ctx
			.gamepad
			.gamepads()?
...
pub fn gamepad(id: GamepadId) -> Result<Gamepad, sdl3::Error> {
	Context::with(|ctx| {
		ctx.gamepad
			.open(id.into())
...
pub fn delta_time() -> Duration {
	Context::with(|ctx| ctx.delta_time)
}
...
pub fn dev_tools_state() -> DevToolsState {
	Context::with(|ctx| ctx.dev_tools_state)
}

pub fn load_font_file(path: impl AsRef<Path>) -> std::io::Result<()> {
	Context::with_mut(|ctx| ctx.text.load_font_file(path))
}

pub fn load_fonts_dir(path: impl AsRef<Path>) {
	Context::with_mut(|ctx| ctx.text.load_fonts_dir(path))
}
