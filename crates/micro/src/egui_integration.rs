...
pub fn draw_egui_output(
	...
	patch_textures(&mut output, textures);
	let scaling_factor = window_scale();
...
fn patch_textures(output: &mut FullOutput, textures: &mut HashMap<egui::TextureId, Texture>) {
	for (texture_id, delta) in output.textures_delta.set.drain() {
		if let Some(texture) = textures.get_mut(&texture_id) {
			...
			if bottom_right.x >= texture.size().x || bottom_right.y >= texture.size().y {
				*texture = texture.resized(bottom_right);
			}
			texture.replace(top_left, &egui_image_data_to_image_buffer(&delta[0].image));
		} else {
			textures.insert(
				texture_id,
				Texture::from_image(
