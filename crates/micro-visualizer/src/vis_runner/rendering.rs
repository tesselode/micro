...
use micro::{graphics::PresentMode, Micro};
use rfd::FileDialog;
...
impl VisRunner {
	pub fn render(&mut self, micro: &mut Micro) {
		...
		micro.set_present_mode(PresentMode::AutoNoVsync);
	}

	pub fn on_rendering_finished(&mut self, micro: &mut Micro) {
		...
		micro.set_present_mode(PresentMode::AutoVsync);
	}
}
