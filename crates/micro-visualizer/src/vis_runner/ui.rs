...
impl VisRunner {
	pub fn render_main_menu_contents(&mut self, micro: &mut Micro, ui: &mut Ui) {
		...
		self.visualizer.menu(micro, ui, self.vis_info());
	}

	pub fn render_rendering_window(&mut self, micro: &mut Micro, egui_ctx: &micro::egui::Context) {
